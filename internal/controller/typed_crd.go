...
func typedResourceCRD(token string, schema *pulumido.PackageSchema) (*apiextensionsv1.CustomResourceDefinition, error) {
	...
	crd := typedCRD(typedGroup, kind, plural, []crdVersion{{name: typedVersion, storage: true}}, spec, nil,
...
func typedCompositeCRD(def string, api *dov1alpha1.CompositeAPI) (*apiextensionsv1.CustomResourceDefinition, error) {
	...
	for _, col := range api.AdditionalPrinterColumns {
		if _, taken := builtinPrinterColumns[strings.ToUpper(col.Name)]; taken {
			return nil, fmt.Errorf("spec.api.additionalPrinterColumns declares %q, which is a built-in column", col.Name)
		}
	}

	crd := typedCRD(compositeAPIGroup(api), api.Kind, compositeAPIPlural(api), versions, spec, api.AdditionalPrinterColumns,
...
// builtinPrinterColumns are the columns every generated kind carries.
var builtinPrinterColumns = map[string]struct{}{"READY": {}, "SYNCED": {}, "REASON": {}, "AGE": {}}
...
func typedCRD(group, kind, plural string, versions []crdVersion,
	spec apiextensionsv1.JSONSchemaProps, extraColumns []apiextensionsv1.CustomResourceColumnDefinition, description string,
	...
	// Author columns sit before AGE so the most useful value (typically an
	// output such as a URL) stays visible on narrow terminals.
	for _, col := range extraColumns {
		columns = append(columns, *col.DeepCopy())
	}
	columns = append(columns, apiextensionsv1.CustomResourceColumnDefinition{Name: "AGE", Type: "date", JSONPath: `.metadata.creationTimestamp`})
