...
The Secret must exist where the runner Job executes: the operator namespace by default, the object's own namespace with `runner.namespaceMode=resource`. `valuesFrom` is unsupported for component resources because their checkpoint would persist the input.

On delete, the runner receives the recorded `status.outputs` so it can remove resources whose provider cannot read them back. Every `valuesFrom` path is stripped from that state first; a secret input never enters a Job spec.
